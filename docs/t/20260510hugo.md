+++
title = "Hugo導入"
date = 2026-05-10T00:00:00+09:00
categories = ["tech"]
tags = ["hugo", "site-generatror"]
+++

このサイトは静的サイトジェネレーターで作って
[GitHub Pages](https://github.com/MichinobuMaeda/MichinobuMaeda.github.io/settings/pages)
で公開しています。ジェネレーターはまず
Jekyll ( [このサイトを GitHub Pages に引っ越し](githubpagesminimal.md) )
、その後 Pyhon
でジェネレーターを自製 ( [Markdown の Mermaid をプラグイン無しで](20230206mermaid.md) )
、Node.js で自製と変えていて、今回 [Hugo](https://gohugo.io/) を導入しました。

ほとんどの作業は VS Code + GitHub Coplilot + Claude Sonnet 4.6 に任せました。まず、

- Markdown のタイトル、日付、カテゴリー、タグを Hugo 用に一括変換
- CSS は流用
- レイアウトを Hugo 用に変更

で、旧版と寸分たがわぬものができました。

次に、タグクラウドでタグをクリックした後に JavaScript
のプログラムで結果を表示していたものを静的なページに置き換えました。これで JavaScript
は不要になります。

サイト生成の所要時間は Node.js 自製版で 1秒だったので、
Hugo で早くなったのかどうかはよくわかりません。
`npm ci` に 2秒かかっていた分は早くなったかな。
